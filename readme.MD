...
Register a hook for keeping track of a form's state. The `name` property of any element it is passed to determines its key in the `form` variable.
...
function MyFormComponent() {
    ...
    return <form onSubmit={
        ...
        <input name='remember' defaultValue={form.remember || false} type='checkbox' onChange={setForm} />
        <button type='submit'>Submit</button>
    </form>;
}
```
...
Automatically prevents common asynchronicity issues by not rerendering the component if the `Promise` resolves when `requestFn` has already been called again. Also will not trigger a re-render with `loading` set to `true` when `requestFn` immediately resolves.
...
function MyFormComponent() {
    const [package, getPackage] = usePromise(packageName => axios.get(`https://www.npmjs.com/package/${packageName}`));

    return <div>
        {
            package.loading ? <p>Loading...</p>
            : package.error ? <p className='red'>An error occurred: {package.error}</p>
            : JSON.stringify(package.data, null 4)
        }
...
function MyFormComponent() {
    const [package, getPackage] = useMemoizedPromise(packageName => axios.get(`https://www.npmjs.com/package/${packageName}`));
...
Create a class function for keeping track of a form's state. The `name` property of any element it is passed to determines its key in the component's state.
...
Create a class function for keeping track of a form's state. The `name` property of any element it is passed to determines the property under which the values will be stored on the component's instance. This method differs from `createFormHandler` in that it doesn't call `setState`, but rather stores the state of the form directly on the class instance.
...
class MyFormComponent extends React.Component {
    ...
    render() {
        ...
        return <form onSubmit={
            ...
            <input name='remember' defaultValue={form.remember || false} type='checkbox' onChange={this.onChange} />
            <button type='submit'>Submit</button>
        </form>;
    }
}
...
```js
import React from 'react';
import usePromise from 'react-toolbelt/createPromiseHandler';
import axios from 'axios';
...
class MyPromiseComponent extends React.Component {
    ...
    getPackage = createPromiseHandler(
        packageName => axios.get(`https://www.npmjs.com/package/${packageName}`,
        'package'
    ).bind(this)
    ...
    render() {
        ...
        return <div>
            ...
            <button type='button' onClick={() => this.getPackage(`react-toolbelt`)}>Go!</button>
        </div>;
    }
}
```
...
```js
import React from 'react';
import usePromise from 'react-toolbelt/createMemoizedPromiseHandler';
import axios from 'axios';
...
class MyPromiseComponent extends React.Component {
    ...
    getPackage = createMemoizedPromiseHandler(
        packageName => axios.get(`https://www.npmjs.com/package/${packageName}`,
        'package'
    ).bind(this)
